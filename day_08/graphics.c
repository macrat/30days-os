...
void draw_image(uint_fast16_t x, uint_fast16_t y, uint_fast16_t width, uint_fast16_t height, uint8_t *image) {
    for (uint_fast32_t i = 0; i < width*height; i++, image++) {
        draw_pixel(x + i%width, y + i/width, *image);
    }
}

void draw_mouse_cursor(uint_fast16_t x, uint_fast16_t y) {
