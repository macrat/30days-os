...
benchmark_t* create_benchmark() {
    ...
    move_window(win, get_screen_width() - 8 * 16 - 40, 40);
    win->extra = (void*)bm;
...
void task_b_main() {
    window_t* win = create_window(30, 20);
    move_window(win, 10, 100);

    static uint8_t i = 0;
    while (1) {
        __asm__("HLT");
        fill_sprite(win->fg, (++i)%16);
    }
}
...
void main() {
    ...
    benchmark_t* bm = create_benchmark();
