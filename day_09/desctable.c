...
} typedef segment_descriptor_t;

struct GateDescriptor {
    uint16_t offset_low, selector;
    uint8_t dw_count, access_right;
    uint16_t offset_high;
} typedef gate_descriptor_t;

void set_segmdesc(segment_descriptor_t* sd, uint32_t limit, uint32_t base, uint16_t ar) {
    if (limit > 0xffff) {
        ar |= 0x8000;  // G_bit = 1
        limit /= 0x1000;
    }
...
void set_gatedesc(gate_descriptor_t* gd, uint32_t offset, uint16_t selector, uint16_t ar) {
